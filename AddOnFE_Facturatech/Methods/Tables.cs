...
namespace AddOnFE_Facturatech.Methods
{
    class Tables
    {
        ...
        private SAPbobsCOM.Company oCompany = Program.SBO_Company;
        ...
        public void leerJsonTablasUsuario()
        {
            try
            {
                string outputJSON = Convert.ToString(dbRE.GetJson("UserTables.json"), CultureInfo.InvariantCulture);
        ...
        public void leerJsonTipOpe()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_TIPOPERA.json"), CultureInfo.InvariantCulture);
                ...
                foreach (var item in dynJson)
                {
                    ...
                    if (!tbl.GetByKey(Convert.ToString(item.Codigo)))
                    {
                        ...
                        if (lRetCode != 0)
                        {
                            ...
                            Log.EscribirLogFileTXT(tbl.TableName + " - " + tbl.Code + ": " + sErrMsg);
                        }
                    }
        ...
        public void leerJsonRespon()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_RESPONSA.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonMedPago()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_MEDPAGO.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonDescu()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_DESCU.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonConcepND()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_CONCEP_ND.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonConcepNC()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_CONCEP_NC.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonTributos()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_TRIBU.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonIdentArti()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_IDENT_ARTI.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonIcoterms()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_INCOTERMS.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonCamposUsuario()
        {
            ...
            try
            {
                string outputJSON = Convert.ToString(dbRE.GetJson("UserFields.json"), CultureInfo.InvariantCulture);
                string validValues = "";
                JArray parsedArray = JArray.Parse(outputJSON);
                var bodyField = "";
                string name = "";
                ...
                foreach (JObject parsedObject in parsedArray.Children<JObject>())
                {
                    ...
                    foreach (JProperty parsedProperty in parsedObject.Properties())
                    {
                        ...
                        {
                            if (oUserFieldsMD.Name == "SEI_FERegFis")
                            {
                                oRS = (SAPbobsCOM.Recordset)oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                                sSql = "Select A0.\"FieldID\" From CUFD A0 " +
                                       "Where A0.\"AliasID\" = '{0}' And A0.\"TableID\" = '{1}'";
                                sSql = string.Format(sSql, oUserFieldsMD.Name, oUserFieldsMD.TableName);
                                oRS.DoQuery(sSql);

                                if (oRS.RecordCount > 0)
                                {
                                    string tabla = "";
                                    tabla = oUserFieldsMD.TableName;

                                    Utilities.Release(oUserFieldsMD);
                                    oUserFieldsMD = null;
                                    GC.Collect();
                                    oUserFieldsMD = (SAPbobsCOM.UserFieldsMD)oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oUserFields);

                                    if (oUserFieldsMD.GetByKey(tabla, (int)oRS.Fields.Item("FieldID").Value))
                                    {
                                        dynamic dynJson1 = JsonConvert.DeserializeObject(value);
                                        ...
                                        if (oUserFieldsMD.ValidValues.Count > 0)
                                        {
                                            for (int a = 0; a < oUserFieldsMD.ValidValues.Count;) //Looping through rows
                                            {
                                                oUserFieldsMD.ValidValues.SetCurrentLine(a);
                                                if (oUserFieldsMD.ValidValues.Value == "04" || oUserFieldsMD.ValidValues.Value == "05")
                                    ...
                                    {
                                        dynamic dynJson2 = JsonConvert.DeserializeObject(value);
                    ...
                    if (lRetCode != 0)
                    {
                        ...
                        Log.EscribirLogFileTXT(oUserFieldsMD.Name + ": " + sErrMsg);
                    }
                    Utilities.Release(oUserFieldsMD);
                    oUserFieldsMD = null;
                    GC.Collect();
        ...
        public void leerJsonDocDIAN()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_CODDOC.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonCfgInter()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_INTERF_CFG.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonUM()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FEDIAN_UM.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
                    tbls = oCompany.UserTables;
        ...
        public void leerJsonUserQueries()
        {
            try
            {
                string inputJSON = Convert.ToString(dbRE.GetJson("UserQueries.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                foreach (var item in dynJson)
                {
        ...
        public void leerJsonFormattedSearches()
        {
            try
            {
                ...
                string inputJSON = Convert.ToString(dbRE.GetJson("FormattedSearches.json"), CultureInfo.InvariantCulture);
                dynamic dynJson = JsonConvert.DeserializeObject(inputJSON);
                ...
                foreach (var item in dynJson)
                {
                    ...
                    if (lRetCode != 0)
                    {
                        ...
                        Log.EscribirLogFileTXT("formID: " + item.FormID + " QueryName: " + item.QueryName + ": " + sErrMsg);
                    }
                    Utilities.Release(oRecordSet);
                    oRecordSet = null;
                    Utilities.Release(oFormattedSearches);
        ...
        public void addUserTables(string description, string table, string type)
        {
            ...
            if (!oUserTablesMD.GetByKey(table))
            {
                ...
                if (lRetCode != 0)
                {
                    ...
                    Log.EscribirLogFileTXT(oUserTablesMD.TableName + ": " + sErrMsg);
                }
                else
                {
            ...
            Utilities.Release(oUserTablesMD);
            oUserTablesMD = null;
            GC.Collect();
        }
        ...
        public void AddQueryManager(string nombreQuery, string categoria)
        {
            try
            {
                ...
                sboQryCategory = (SAPbobsCOM.QueryCategories)oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oQueryCategories);

                SAPbobsCOM.UserQueries sboUserQuery;
                sboUserQuery = (SAPbobsCOM.UserQueries)oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oUserQueries);
                ...
                codigoSubCat = (int)oRS.Fields.Item("CategoryId").Value;
                ...
                Utilities.Release(sboQryCategory);
                sboQryCategory = null;
                Utilities.Release(sboUserQuery);
                sboUserQuery = null;
                Utilities.Release(oRS);
                oRS = null;
                GC.Collect();
            ...
            {
                Log.EscribirLogFileTXT("AddQueryManager: " + ex.Message);
            }
        }
        ...
        public void CreateUDO()
        {
            ...
            if (!retval)
            {
                ...
                Utilities.Release(oUserObjectMD);
                GC.Collect();
            }
        }
